...
public enum BeanStringStyle {
	...
	static StringBuffer beanToString(Object bean, String childBeanName) {
		...
		for (int i = 0, length = beanFields.length; i < length; i++) {
			Field beanField = beanFields[i];
			beanField.setAccessible(true);
			String fieldName = childBeanName == null ? beanField.getName() : childBeanName + beanField.getName();
	...
	private static void fieldToString(StringBuffer stringBuffer, String fieldName, Object fieldValue, int level, boolean isLast) {
		Class<?> fieldType = fieldValue.getClass();
		Integer index = BeanUtil.commonTypeMap.get(fieldType);
		if (index != null) {
			switch (index) {
			case 10:
				writeLine(stringBuffer, level, fieldName + "=" + DateUtil.dateToString((Date) fieldValue, DateStyle.YYYY_MM_DD_HH_MM_SS),
						isLast);
				break;
			case 13:
			case 14:
			...
			case 16:
				...
				for (Entry<?, ?> entry : map.entrySet()) {
					fieldToString(stringBuffer, entry.getKey().toString(), entry.getValue(), level != -1 ? level + 1 : level, isLast);
				}
	...
	private static void fieldToString(StringBuffer stringBuffer, String fieldName, Object fieldValue) {
		...
		if (index != null) {
			switch (index) {
			case 10:
				stringBuffer.append(fieldName).append("=").append(DateUtil.dateToString((Date) fieldValue, DateStyle.YYYY_MM_DD_HH_MM_SS))
						.append("&");
				break;
			case 13:
			case 14:
	...
	private static void arrayFieldToString(StringBuffer stringBuffer, String fieldName, Object fieldValue, int level, boolean isLast) {
		if (fieldName != null) {
			writeLine(stringBuffer, level, fieldName + "={", true);
		} else {
			writeLine(stringBuffer, level, "{", true);
