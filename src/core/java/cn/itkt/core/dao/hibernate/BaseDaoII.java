...
@Repository
public class BaseDaoII<PK extends Serializable> implements IBaseDaoII<PK> {
	...
	public <T> T get(Class<T> entityClass, PK id) {
		return (T) sessionFactory2.getCurrentSession().get(entityClass, id);
	}

	@SuppressWarnings("unchecked")
	@Override
	public <T> T load(Class<T> entityClass, PK id) {
		return (T) sessionFactory2.getCurrentSession().load(entityClass, id);
	}

	@SuppressWarnings("unchecked")
	@Override
	public <T> List<T> findAll(Class<T> entityClass) {
		return sessionFactory2.getCurrentSession().createQuery("from " + entityClass.getName()).list();
	}
...
//			sessionFactory.getCurrentSession() ;
			Query hqlQuery = sessionFactory2.getCurrentSession().createQuery(queryString);
		...
		if (!BeanUtil.isNull(sql)) {
			SQLQuery sqlQuery = sessionFactory2.getCurrentSession().createSQLQuery(sql);
	...
	public <T> PK save(T entity) {
		return (PK) sessionFactory2.getCurrentSession().save(entity);
	}

	@Override
	public <T> void update(T entity) {
		sessionFactory2.getCurrentSession().update(entity);
	}
	...
	public <T> void saveOrUpdate(T entity) {
		sessionFactory2.getCurrentSession().saveOrUpdate(entity);
	}
	...
	public <T> void delete(T entity) {
		sessionFactory2.getCurrentSession().delete(entity);
	}

	@Override
	public <T> void delete(Class<T> entityClass, PK id) {
		T t = this.get(entityClass, id);
		sessionFactory2.getCurrentSession().delete(t);
	}
    ...
    protected void appendConditionValues(Query query, List<Condition> conditions) {
        ...
        for (Condition condition : conditions) {
            if (condition.getSignal().trim().equalsIgnoreCase("in")
                    || condition.getSignal().trim().equalsIgnoreCase("not in")) {
                query.setParameterList(this.generateParamName(condition), (List<?>) condition.getValue());
            } else if (condition.getSignal().trim().equalsIgnoreCase("is")
                    || condition.getSignal().trim().equalsIgnoreCase("is not") 
                ...
                query.setParameter(this.generateParamName(condition), condition.getValue());
            }
        }
    }
